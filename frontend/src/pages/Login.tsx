...
import { AlertCircle, Lock, User as UserIcon, Zap } from 'lucide-react';
...
export default function Login() {
  ...
  return (
    <div className="min-h-screen bg-base text-primary flex items-center justify-center p-4 relative overflow-hidden">
      {/* Background Decor */}
      <div className="absolute top-[-10%] left-[-10%] w-[40%] h-[40%] bg-emerald/5 rounded-full blur-[120px] pointer-events-none" />
      <div className="absolute bottom-[-10%] right-[-10%] w-[40%] h-[40%] bg-cyan/5 rounded-full blur-[120px] pointer-events-none" />
      ...
      <div className="max-w-md w-full glass-card p-10 relative z-10 border border-dim shadow-2xl animate-page-in">
        <div className="text-center mb-10">
          ...
          <h2 className="text-3xl font-display font-black tracking-tighter uppercase leading-none">
            APEX <span className="text-emerald glow-emerald">TERMINAL</span>
          </h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="space-y-2">
            <label className="font-data-tiny text-muted uppercase tracking-widest ml-1">Username / Email</label>
          ...
          >
            {loading ? (
              <><Zap className="w-4 h-4 animate-spin" /> AUTHENTICATING...</>
            ) : (
              <><Zap className="w-4 h-4 group-hover:fill-emerald transition-all" /> ACCESS TERMINAL</>
            )}
          </button>
        </form>
        ...
        <div className="mt-10 text-center">
          <p className="text-xs font-data-small text-muted uppercase tracking-widest">
            New operative? <Link to="/signup" className="text-emerald hover:text-white transition-colors ml-1 font-bold underline decoration-emerald/30 underline-offset-4">Create Account</Link>
          </p>
        </div>
      </div>
    </div>
