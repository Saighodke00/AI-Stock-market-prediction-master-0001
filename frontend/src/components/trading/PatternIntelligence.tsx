...
export const PatternIntelligence: React.FC<PatternIntelligenceProps> = ({ ticker, mode, patterns: propPatterns }) => {
    ...
    if (loading) {
        return (
            <div className="p-10 flex flex-col items-center justify-center gap-4 bg-white/[0.02] border border-dim rounded-2xl animate-pulse">
        ...
        <div className="flex flex-col gap-4">
            <div className="flex items-center justify-between px-1">
                <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-[10px] font-bold text-muted tracking-[0.2em] uppercase font-body">Geometric Signatures</h3>
                </div>
                <span className="text-[9px] font-black text-muted tracking-widest uppercase">{patterns.length} DETECTIONS</span>
            </div>

            <div className="flex flex-col gap-3">
                {patterns.length > 0 ? (
                    patterns.map((p, i) => (
                        <div key={i} className="bg-white/[0.02] border border-dim hover:bg-white/[0.04] hover:border-cyan-500/20 p-4 rounded-xl transition-all duration-300 group">
                            <div className="flex justify-between items-start mb-3">
                                ...
                                <div className="p-2 bg-white/[0.03] rounded-lg border border-dim">
                                    <span className="text-lg">{p.emoji || '📐'}</span>
                                </div>
                            </div>

                            {p.description && (
                                <p className="text-[11px] text-secondary mb-3 leading-relaxed font-body italic">
                                    {p.description}
                                </p>
                            )}
                            ...
                            {(p.target || p.breakout) && (
                                <div className="grid grid-cols-2 gap-2 mb-4">
                                    {p.breakout && (
                                        <div className="bg-void/50 p-2 rounded border border-dim">
                                            <div className="text-[8px] text-muted uppercase tracking-tighter mb-0.5">Level</div>
                    ...
                    <div className="py-12 text-center bg-white/[0.01] border border-dim border-dashed rounded-2xl">
                        ...
                        <p className="text-muted font-body text-[9px] font-bold tracking-widest uppercase px-6 leading-relaxed">
