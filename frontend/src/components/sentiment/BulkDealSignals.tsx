...
export const BulkDealSignals: React.FC<BulkDealSignalsProps> = ({ deals, signal, summary, isLoading }) => {
  ...
  return (
    <div className="p-5 bg-[#0a0f1d] border border-white/5 rounded-xl h-full flex flex-col relative overflow-hidden group">
      <div className="flex justify-between items-start mb-6 z-10">
        <div>
          <h4 className="text-[10px] font-black text-slate-500 tracking-[0.2em] uppercase mb-1">Institutional Deal Matrix</h4>
          <p className="text-[9px] font-mono text-slate-500 max-w-[200px]">{summary}</p>
        </div>
      ...
      <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar">
        <table className="w-full text-left font-mono">
          <thead className="text-[8px] text-slate-600 border-b border-white/10 uppercase tracking-widest">
            <tr>
          ...
          <tbody className="text-[10px]">
            {deals.length === 0 ? (
              <tr>
                <td colSpan={3} className="py-8 text-center text-slate-600 italic">NO LARGE BLOCK DEALS DETECTED (7D)</td>
              </tr>
            ) : (
              deals.map((deal, idx) => (
                <tr key={idx} className="border-b border-white/[0.02] hover:bg-white/[0.02] transition-colors group/row">
                  <td className="py-3 pr-2">
                    <div className="flex flex-col">
                      <span className="text-slate-300 font-bold group-hover/row:text-white transition-colors truncate max-w-[140px]">
                        {deal.client_name}
                      </span>
                      <span className="text-[8px] text-slate-500 uppercase">{deal.date}</span>
                    </div>
                  </td>
                  <td className="py-3">
                  ...
                  <td className="py-3 text-right">
                    <div className="flex flex-col items-end">
                      <span className="text-slate-300 font-bold">{(deal.quantity / 1000000).toFixed(2)}M</span>
                      <span className="text-[8px] text-slate-500">@ ₹{deal.price.toFixed(0)}</span>
                    </div>
                  </td>
                </tr>
              ))
      ...
      <div className="mt-4 pt-3 border-t border-white/5 flex items-center gap-2">
        ...
        <span className="text-[8px] font-mono text-slate-600 uppercase tracking-widest font-black">Verified SEBI Source</span>
      </div>
    </div>
  );
};
