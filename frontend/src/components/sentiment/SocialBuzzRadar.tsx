...
export const SocialBuzzRadar: React.FC<SocialBuzzRadarProps> = ({ data, isLoading }) => {
  ...
  return (
    <div className="p-5 bg-[#0a0f1d] border border-dim rounded-xl h-full flex flex-col gap-6 relative overflow-hidden group">
      ...
      <div className="flex justify-between items-end z-10">
        <div>
          <h4 className="text-[10px] font-black text-muted tracking-[0.2em] uppercase mb-1">Social Momentum Radar</h4>
          <div className="text-2xl font-display font-black text-white">
            {avgScore > 0 ? '+' : ''}{avgScore.toFixed(2)}
          </div>
        </div>
        <div className="text-right">
          <span className="text-[9px] font-mono text-muted block">TOTAL VOLUME</span>
          ...
          <div key={source.name} className="flex flex-col gap-2">
            <div className="flex justify-between items-center text-[10px] font-mono font-bold">
              <div className="flex items-center gap-2">
                ...
                <span className="text-secondary uppercase tracking-widest">{source.name}</span>
              </div>
      ...
      <div className="mt-auto pt-4 border-t border-dim grid grid-cols-2 gap-4 z-10">
         <div className="bg-white/5 p-2 rounded-lg text-center">
            <span className="text-[8px] text-muted block uppercase font-black">Velocity</span>
            <span className="text-xs font-mono font-bold text-emerald-400">STABLE</span>
         </div>
         <div className="bg-white/5 p-2 rounded-lg text-center">
            <span className="text-[8px] text-muted block uppercase font-black">Confluence</span>
