...
def fetch_multi_modal(ticker: str, period: str = "2y") -> pd.DataFrame:
    ...
    for yticker, col_name in MACRO_TICKERS.items():
        logger.info("Fetching macro index %s → '%s'…", yticker, col_name)
        try:
            ...
            if macro_raw is None or macro_raw.empty:
                logger.warning(
                    "Empty data for macro index %s — column '%s' will be NaN.",
                    yticker, col_name,
                )
                df[col_name] = np.nan
                continue
            ...
            # Guard against yfinance returning a DataFrame instead of Series
            ...
            # Left-join on the primary ticker's index
            df = df.join(close_series, how="left")

        except Exception as exc:
            logger.warning(
                "Could not fetch macro index %s (%s) — column '%s' will be NaN.",
                yticker, exc, col_name,
            )
            df[col_name] = np.nan
